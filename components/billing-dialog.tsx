      ...
      >
        <div className="px-5 pt-6 pb-5 sm:px-6">
          <SheetHeader className="mb-4">
            <SheetTitle className="text-2xl font-semibold">Billing</SheetTitle>
            <SheetDescription className="text-sm text-white/75">
              Manage your credits, top-ups, and billing settings
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="mt-2 p-3 rounded border border-[color:var(--border)]">
            ...
            {/* Tiered pricing display for Pro/Ultra users */}
            {(plan === "pro" || plan === "ultra") && (
            ...
            <div className="flex items-center gap-2">
              ...
              <Button disabled={loading} onClick={startTopup}>
                {topup && Number(topup) > 0 ? `Refuel for $${Math.max(5, Math.floor(Number(topup)))}` : 'Refuel'}
              </Button>
            </div>
            <div className="text-xs text-white/60 mt-2">
              {plan === "pro" || plan === "ultra" ? "Choose a tier above or enter a custom amount (min $5)." : "Minimum $5 per purchase."}
            </div>
