...
const debounce = <F extends (...args: unknown[]) => void>(fn: F, wait: number) => {
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const mobileKey = (`mobile${cap}`) as 'mobileHeight' | 'mobileWidth';
      const tabletKey = (`tablet${cap}`) as 'tabletHeight' | 'tabletWidth';
      const desktopKey = (`desktop${cap}`) as 'desktopHeight' | 'desktopWidth';
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      ...
      zIndex: isPageTarget ? (config.zIndex ? config.zIndex + 100 : 100) : config.zIndex,
      ...config.style
    };
    ...
    if (isVertical) {
      ...
      const posKey = config.position;
      if (posKey === 'top' || posKey === 'bottom') {
        baseStyle[posKey] = 0;
      }
      baseStyle.left = 0;
      baseStyle.right = 0;
...
// Attach static references with typed indexing to avoid any
(GradualBlurMemo as unknown as { [key: string]: unknown }).PRESETS = PRESETS;
(GradualBlurMemo as unknown as { [key: string]: unknown }).CURVE_FUNCTIONS = CURVE_FUNCTIONS;
