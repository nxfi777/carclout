    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          try {
            const payloadObj = (item as unknown as Record<string, unknown>)?.payload as
              | (Record<string, unknown> & { fill?: unknown })
              | undefined;
            const fillVal = payloadObj?.fill;
            if (typeof fillVal === "string") indicatorColor = fillVal;
          } catch {}
          return (
            <div
            ...
            >
              {formatter && item?.value !== undefined && item.name ? (
                formatter(
                  ...
                  index,
                  (((item as unknown as Record<string, unknown>).payload || {}) as Record<string, unknown>)
                )
              ) : (
                <>
                  {itemConfig?.icon ? (
...
function getPayloadConfigFromPayload(
  ...
  return configLabelKey in config ? config[configLabelKey] : (config as ChartConfig)[key as keyof ChartConfig];
}
