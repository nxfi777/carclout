...
export default function ContactPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) throw new Error((data && data.error) || "Failed to send message");
      toast.success("Message sent! We'll get back to you soon.");
      setName("");
      setEmail("");
      setMessage("");
    } catch (err: unknown) {
      try { console.error("Contact error", err); } catch {}
        ...
        <form onSubmit={onSubmit} className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--popover)]/70 backdrop-blur p-[1rem] md:p-[1.25rem] space-y-[0.9rem]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[0.75rem]">
            <div className="space-y-[0.35rem]">
              <label htmlFor="name" className="text-sm text-[color:var(--foreground)]/80">Name</label>
              <Input id="name" value={name} onChange={(e)=>setName(e.target.value)} placeholder="Your name" className="h-10" required />
            </div>
            <div className="space-y-[0.35rem]">
              <label htmlFor="email" className="text-sm text-[color:var(--foreground)]/80">Email</label>
          ...
          <div className="space-y-[0.35rem]">
            <label htmlFor="message" className="text-sm text-[color:var(--foreground)]/80">Message</label>
          ...
          <p className="text-xs text-[color:var(--foreground)]/60">Or email us directly at <a className="underline hover:text-[color:var(--primary)]" href="mailto:support@nytforge.com">support@nytforge.com</a>.</p>
        </form>
      </section>
    </main>
  );
