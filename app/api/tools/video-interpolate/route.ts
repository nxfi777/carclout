...
export async function POST(req: Request) {
  try {
    ...
    */
    ...
    // Generate and store blurhash for interpolated video (non-fatal)
    try {
      ...
      console.log(`Stored library video metadata for interpolated video ${outputKey}`);
    } catch (error) {
      console.error('Failed to store interpolated video metadata (non-fatal):', error);
    }
